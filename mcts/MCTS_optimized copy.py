"""
优化版 MCTS - 提速10倍+
已移除不合理的简化策略（中心优先等），Rollout 采用纯随机策略。
保留了改进的终局评估逻辑（潜力奖励）。
"""
...
class FastMCTS:
    ...
    def _rollout(self, state: FastGameState) -> float:
        ...
        while not state.is_terminal and depth < FastConfig.MAX_SIMULATION_DEPTH:
            ...
            # 原始：action = FastPolicy.select_action(state)
            # 优化：替换为纯随机选择
                ...
                if fruit > 0 and fruit < 10:
                    ...
                    for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                        ...
                        if 0 <= nr < state.height and 0 <= nc < state.width:
                            ...
                            # 奖励 A: 相邻有相同的水果 (立即合并潜力)
                            if neighbor == fruit:
                                potential_reward += 0.5 * fruit
                            
                            # 奖励 B: 相邻有大一级的水果 (未来合并潜力)
                            if neighbor == fruit + 1:
                                potential_reward += 1.0 * fruit
                                
        value += potential_reward * 2.0  # 将潜力奖励加权后计入总值
...
class FastMCTSAgent:
    ...
    def _convert_state(self, game_interface) -> FastGameState:
        ...
        # 警告：此处的 game_interface 依赖于外部环境，在独立运行时会报错
        # 保持原样以供外部环境调用
...
if __name__ == "__main__":
    ...
    print("\n优化效果: Rollout 策略已替换为纯随机，专注于树搜索的统计精度。")
