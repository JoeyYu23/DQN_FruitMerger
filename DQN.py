...
for d in [BUFFER_DIR, OUTPUT_DIR]:
    if not os.path.exists(d):
        os.mkdir(d)

FINAL_PARAM_PATH = os.path.join(OUTPUT_DIR,"final.pdparams")
...
if __name__ == "__main__":
    ...
    # ========= 创建带时间戳的运行文件夹 =========
    timestamp = time.strftime("%Y-%m-%d_%H-%M-%S")  # e.g. 2025-11-12_15-43-27
    run_dir = os.path.join(OUTPUT_DIR, timestamp)
    weight_dir = os.path.join(run_dir, "models")
    os.makedirs(weight_dir, exist_ok=True)

    history_path = os.path.join(run_dir, "training_history.csv")
    print(f"Current run folder: {run_dir}")
    ...
    for episode_id in range(0, max_episode + 1):
        ...
        if len(memory) >= MEMORY_WARMUP_SIZE:
            avg_loss = getattr(agent, "last_loss", np.nan)

        if episode_id % episode_per_save == 0:
            model_path = os.path.join(weight_dir, f"episode_{episode_id}.pdparams")
            paddle.save(agent.policy_net.state_dict(), model_path)
            print(f"Saved model to {model_path}")

            print(f"Episode {episode_id} | Epsilon={agent.e_greed:.3f} | Reward={total_reward:.2f}")
            score = compare_with_random(env, agent, action_dim)

            history["episode"].append(episode_id)
            history["reward_sum"].append(total_reward)
            history["epsilon"].append(agent.e_greed)
            history["loss"].append(avg_loss)
            history["score"].append(score)

            torch.save(list(memory), os.path.join(BUFFER_DIR, "replay_buffer.pt"))
            print(f"[INFO] Saved replay buffer with {len(memory)} transitions")



    final_model_path = os.path.join(weight_dir, "final_model.pdparams")
    ...
    torch.save(list(memory), os.path.join(BUFFER_DIR, "replay_buffer_final.pt"))
    print("[INFO] Final replay buffer saved.")
